...
#GIT Stuff
alias gs="git status"
alias ga="git add" #ga <file name>. Add file name after ga
...
alias gm="git commit -m" #Commits current queue to git. Requires you to type a commit message
...
#Pull everything down!!! (Experimental)
alias pullall="for REPO in `find ~/git -maxdepth 1 -type d`; do (cd "$REPO" && git pull origin master); done;"
