...
while true; do # This simply prompts the user if they want the script to install git, zsh and vim
    read -e -p "Would you like me to install them for you? (Y/n)\n" -i "Y" yn
    case $yn in
        [Yy]* ) sudo apt-get -y install git zsh vim;
            printf "Ok I am installing git, zsh and vim for you\n"; break;; 
        [Nn]* ) printf "Ok, skipping the installs\n"; break;;
...
if [[ ! -d ~/git/ ]] || [[ ! -d ~/.git/ ]]; then
    while true; do
        read -e -p "Do you want a reguler folder for git or a hidden folder? (R/h)\n" -i "R" rh
        case $rh in
            [Rr]* ) if [[ ! -d ~/git/ ]]; then 
...
while true; do 
    read -e -p "Would you like me to change this for you? (Y/n)\n" -i "Y" yn
    case $yn in
        [Yy]* ) printf "Your shell is now ZSH!"; chsh -s $(which zsh); break;;
        [Nn]* ) printf "Ok, I am not setting zsh as your shell\n"; break;;
